...
func (r *TodoItemPostgres) Create(listId int, item TO_DO_List.TodoItem) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]TO_DO_List.TodoItem, error) {
	var items []TO_DO_List.TodoItem
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (TO_DO_List.TodoItem, error) {
	var item TO_DO_List.TodoItem
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li ON li.item_id = ti.id 
...
func (r *TodoItemPostgres) Update(userId, itemId int, input TO_DO_List.UpdateItemInput) error {
